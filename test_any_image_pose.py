...
def main(args):
    ...
    if args.image_pretrain_dir.endswith(".th"):
        print(f'find model state dict from {args.image_pretrain_dir} ...')
        if args.local_rank == 0:
            assert hexists(args.image_pretrain_dir)
            load_state_dict(model, args.image_pretrain_dir,strict=False) 
    else:
        print(f'find optimizer state dict from {os.path.join(args.image_pretrain_dir, "optimizer_state_latest.th")} ...')
    ...
    for itr in range(args.num_train_steps):
        ...
        with torch.no_grad():
            print("args:",args)
            nSample = 1  # Force single sample processing
